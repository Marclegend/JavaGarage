...
public class Moto extends Vehiculo  implements Refilleable {

    public Moto(int numRuedas , int gasolina, int tamano, String modeloVehiculo, int gasolinaMAX, Cliente cliente) {
    ...
    public void Refill(Cliente cliente) { //Esta función pedirá el cliente para poder cobrarle
        System.out.println("Tu moto tiene " + this.gasolina + " litros de gasolina,¡Vamos a aumentarlo!"); // Informamos de la gasolina actual
        if (this.gasolinaMAX <= this.gasolina) { //Si tiene más gasolina que la máxima, o igual, no le rellenaremos el tanque
            System.out.println("¡Hey, el depósito de tu moto ya está lleno y no se le puede meter más gasolina!");
        } else {
            ...
            if (this.cliente.getDinero() < precio) { //Veremos si tiene el dinero suficiente para pagar el refill
                ...
                System.out.println("¡Ahora tu moto tiene " + this.gasolina + " litros de gasolina en el depósito!");
            }
